...
export class ConexionDBComponent implements OnInit {
  ...
  configBDForm = new FormGroup({
    server: new FormControl(localStorage.getItem('serverDB'), Validators.required),
    database: new FormControl(localStorage.getItem('nameDB'), Validators.required),
    userName: new FormControl(localStorage.getItem('userDB'), Validators.required),
    password: new FormControl(this.api.DesEncriptar(localStorage.getItem('passDB'),1234), Validators.required),
    instanceName: new FormControl(localStorage.getItem('intanceDB'))
  })
...
@Output() formEvent = new EventEmitter<any>();
  ...
  onCheckConection(form: any) {
    ...
    this.api.SendParametros(form).subscribe(
        ...
        if (this.resultCheckConecction !== '') this.requestCheckConecction = false;
        ...
        if(this.resultCheckConecction=='Conexion correcta..')
        {
          ...
          localStorage.setItem('passDB',this.api.Encriptar(form['password'],1234));
  ...
  openDialog(rs: string /*lo q dice el dialogo*/, d: string /*el tipo de dialogo*/) {
